...
//globals
let listStopWatch = [];  
let idCounter = 1;  //gives id to the watch object
...
function StopWatchBody(Watch) {
    let pausePlayButtonStr = function(status) {
        if(status == 1) {
    ...
    let retStr = "<div class='col-md-6' id='" + Watch.id + "'>" +
...
function updateClocks() {
    for(let i = 0; i < listStopWatch.length; i++) {
        if(listStopWatch[i].status !== 0) {
            ...
            let tempId = 'watch' + listStopWatch[i].id;
            let tempModalId = 'note' + listStopWatch[i].id;
